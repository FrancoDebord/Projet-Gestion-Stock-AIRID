...
<div class="container py-4">
    <div class="row justify-content-center">
        <div class="col-lg-10">
            <h1 class="h4 mb-3">Nouvelle Demande de Stock</h1>

            <div class="card shadow-sm p-4">
                <form action="{{ route('stock-requests.store') }}" method="POST" id="requestForm">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <input type="datetime-local" name="request_date" value="{{ old('request_date', now()->format('Y-m-d\TH:i')) }}" required class="form-control @error('request_date') is-invalid @enderror">
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" name="code_machine" value="{{ old('code_machine') }}" class="form-control @error('code_machine') is-invalid @enderror" maxlength="100">
                            @error('code_machine')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>

                        <div class="col-md-3">
                            <label class="form-label">Numéro de Bureau</label>
                            <input type="text" name="room_number" value="{{ old('room_number') }}" class="form-control @error('room_number') is-invalid @enderror" maxlength="50">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="general_notes" id="general_notes" rows="3" class="form-control @error('general_notes') is-invalid @enderror">{{ old('general_notes') }}</textarea>
                        @error('general_notes')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <h5 class="mb-3">Détails des Produits Demandés</h5>
                    <div id="detailsContainer">
                        @for($index = 0; $index < max(1, count(old('details', [0]))); $index++)
                        <div class="detail-row border rounded p-3 mb-3" data-index="{{ $index }}">
                            <div class="row g-3">
                                <div class="col-md-4">
                                    ...
                                    <select name="details[{{ $index }}][stock_item_id]" class="form-select @error('details.'.$index.'.stock_item_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($stockItems as $item)
                                            <option value="{{ $item->id }}" @if(old('details.'.$index.'.stock_item_id') == $item->id) selected @endif>{{ $item->name }} ({{ $item->unit }})</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="number" name="details[{{ $index }}][requested_quantity]" value="{{ old('details.'.$index.'.requested_quantity') }}" class="form-control @error('details.'.$index.'.requested_quantity') is-invalid @enderror" min="1" required>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="details[{{ $index }}][request_reason]" value="{{ old('details.'.$index.'.request_reason') }}" class="form-control @error('details.'.$index.'.request_reason') is-invalid @enderror" required>
                            ...
                            <div class="row g-3 mt-2">
                                <div class="col-12">
                                    ...
                                    <textarea name="details[{{ $index }}][usage_description]" rows="2" class="form-control">{{ old('details.'.$index.'.usage_description') }}</textarea>
                                </div>
                            </div>
                            <button type="button" class="btn btn-outline-danger btn-sm mt-2 remove-detail-btn" style="{{ count(old('details', [0])) <= 1 ? 'display: none;' : '' }}">
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        <a href="{{ route('stock-requests.index') }}" class="btn btn-light">Annuler</a>
...
document.addEventListener('DOMContentLoaded', function() {
    let detailIndex = {{ max(1, count(old('details', [0]))) }};
