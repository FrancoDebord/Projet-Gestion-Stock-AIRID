...
<div class="container py-4">
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <h1 class="h4 mb-3">Modifier Réception</h1>

            <div class="card shadow-sm p-4">
                <form action="{{ route('stock-arrivals-admin.update', $stockArrivalAdministration) }}" method="POST" enctype="multipart/form-data">
                    @csrf @method('PATCH')

                    <div class="mb-3">
                        <label class="form-label">Date de Réception *</label>
                        <input type="datetime-local" name="date_arrival" value="{{ old('date_arrival', $stockArrivalAdministration->date_arrival->format('Y-m-d\TH:i')) }}" required class="form-control @error('date_arrival') is-invalid @enderror">
                        @error('date_arrival')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Expéditeur</label>
                        <input type="text" name="sender" value="{{ old('sender', $stockArrivalAdministration->sender) }}" class="form-control @error('sender') is-invalid @enderror">
                        @error('sender')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Description Globale</label>
                        <textarea name="description_globale" id="description_globale" rows="5" class="form-control @error('description_globale') is-invalid @enderror">{{ old('description_globale', $stockArrivalAdministration->description_globale) }}</textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="stock_location_destination" class="form-select @error('stock_location_destination') is-invalid @enderror" required>
                            ...
                            @foreach($locations as $loc)
                                <option value="{{ $loc->id }}" @if(old('stock_location_destination', $stockArrivalAdministration->stock_location_destination) == $loc->id) selected @endif>{{ $loc->stock_name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select name="administration_staff" class="form-select @error('administration_staff') is-invalid @enderror" required>
                                <option value="">— Sélectionner —</option>
                                @foreach($users as $u)
                                    <option value="{{ $u->id }}" @if(old('administration_staff', $stockArrivalAdministration->administration_staff) == $u->id) selected @endif>{{ $u->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="staff_transmis_stock" class="form-select @error('staff_transmis_stock') is-invalid @enderror">
                                <option value="">— Sélectionner —</option>
                                @foreach($users as $u)
                                    <option value="{{ $u->id }}" @if(old('staff_transmis_stock', $stockArrivalAdministration->staff_transmis_stock) == $u->id) selected @endif>{{ $u->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        @if($stockArrivalAdministration->bordereau_delivery)
                            <div class="mb-2"><a href="{{ asset('storage/' . $stockArrivalAdministration->bordereau_delivery) }}" target="_blank">Voir</a></div>
                        @endif
                    ...
                    <div class="mb-3">
                        ...
                        @if($stockArrivalAdministration->certificate_analysis)
                            <div class="mb-2"><a href="{{ asset('storage/' . $stockArrivalAdministration->certificate_analysis) }}" target="_blank">Voir</a></div>
                        @endif
                    ...
                    <div class="mb-3">
                        ...
                        @if($stockArrivalAdministration->msds)
                            <div class="mb-2"><a href="{{ asset('storage/' . $stockArrivalAdministration->msds) }}" target="_blank">Voir</a></div>
                        @endif
                    ...
                    <div class="mb-3">
                        ...
                        @if($stockArrivalAdministration->other_document)
                            <div class="mb-2"><a href="{{ asset('storage/' . $stockArrivalAdministration->other_document) }}" target="_blank">Voir</a></div>
                        @endif
                    ...
                    <div class="d-flex justify-content-end gap-2 mt-3">
                        <a href="{{ route('stock-arrivals-admin.show', $stockArrivalAdministration) }}" class="btn btn-light">Annuler</a>
